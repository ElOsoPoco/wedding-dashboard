    ...
    private void processLine(String line) {
        ...
        switch(key) {
            case "db_user": this.dbUser = value;
                            Logger.getLogger(DatabaseProperties.class.getName()).log(Level.INFO, "Read dbUser as: {0}", this.dbUser);
                            break;
            case "password": this.password = value;
                            Logger.getLogger(DatabaseProperties.class.getName()).log(Level.INFO,
                            "Read password as: *******");
                            break;
            case "db_url": this.dbURL = value;
                            Logger.getLogger(DatabaseProperties.class.getName()).log(Level.INFO, "Read dbURL as: {0}", this.dbURL);
                            break;            
        }
    }
